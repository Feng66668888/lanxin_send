...
def send_file(channel_button, file_path):
...
def do_police():
    ...
    logger.info('遍历'+CONF['APP']['SAVE_DIR'])
    files = os.listdir(CONF['APP']['SAVE_DIR'])
    print(files)
    if len(files) > 0:
        for f in files:
            #if re.match(r'^txt$', f):
          ...
          #      new_name = 'xx_{}_{}.txt'.format(f[-14:-6], f[-6:-4])
                ...
                fullfilena=os.path.join(CONF['SOURCE']['SAVE_DIR'], f)
                print(fullfilena)
                logger.info(fullfilena)
                send_file(CONF['LANXIN']['CHANNEL_TARGET'], fullfilena)

                time.sleep(10)  # 避免连续发送失败
            #else:  # 文件名称不符合情况
